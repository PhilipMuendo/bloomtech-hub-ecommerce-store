// Final clear and re-import with correct table name
...
const finalClearAndReimport = async () => {
  try {
    console.log('🧹 Final clearing of products table...');
    
    // Disable foreign key checks temporarily
    await sequelize.query('SET FOREIGN_KEY_CHECKS = 0');
    
    // Truncate the table to completely clear it and reset auto-increment
    await sequelize.query('TRUNCATE TABLE products');
    
    // Re-enable foreign key checks
    await sequelize.query('SET FOREIGN_KEY_CHECKS = 1');
    
    console.log('✅ Products table cleared and auto-increment reset');
