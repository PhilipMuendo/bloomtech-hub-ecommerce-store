// Force clear database and re-import all products
...
const forceClearAndReimport = async () => {
  try {
    console.log('🧹 Force clearing all products from database...');
    
    // Disable foreign key checks temporarily
    await sequelize.query('SET FOREIGN_KEY_CHECKS = 0');
    
    // Truncate the table to completely clear it
    await sequelize.query('TRUNCATE TABLE Products');
    
    // Re-enable foreign key checks
    await sequelize.query('SET FOREIGN_KEY_CHECKS = 1');
    
    console.log('✅ All products force deleted');
